...
function EVAL(ast, env::MalEnv)
    while true
        ...
        elseif first === Symbol("try*")
            try_block = args[1]

            try
                return EVAL(try_block, env)
            catch e
                err = if e isa MalException
                    e.value
                elseif e isa ErrorException
                    e.msg
                else
                    error("Unexpected exception thrown from `try` expression.")
                end

                if length(args) >= 2 && args[2][1] === Symbol("catch*")
                    catch_exception_name = args[2][2]
                    catch_body = args[2][3]
                    Env.set!(env, catch_exception_name, err)
                    return EVAL(catch_body, env)
                else
                    # if there is no `catch*` expression then re-throw the exception
                    throw(e)
                end
            end
        else
    ...
    while true
        ...
        try
            ...
            if e isa ErrorException
                println("Error: $(e.msg)")
            elseif e isa MalException
                println("Exception: $(e.value)")
            else
                println(e)
            end
        end
